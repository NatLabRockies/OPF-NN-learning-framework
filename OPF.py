...
# float 64 is necessary for KKT
torch.set_default_dtype(torch.float64)

parser = argparse.ArgumentParser(description='ACOPF Solver')
parser.add_argument('--epochs', type=int, default=100, help='number of neural network epochs')
...
class ACOPFProblem:
    def __init__(self, data, ppc, train_frac=0.7):
    ...
    def gen_map_pv(self):
        ...
        slack_mask = gen_bus_full == self.slack.item()
        non_slack_gen_idx = np.where(~slack_mask)[0]
        gen_bus_nonslack = gen_bus_full[non_slack_gen_idx]
        g_slack = np.where(gen_bus_full == self.slack)[0][0]

        return gen_bus_idx, gen_bus_unique, gen_bus_full, non_slack_gen_idx, gen_bus_nonslack, g_slack

def build_required_inputs(Pd, Qd, Pg_DC, Qg, Vm, Va):
    # read mat file to ppc.
    ppc = scipy.io.loadmat(os.path.join('case_ACTIVSg2000.mat'))

    ppc['gen'][:, 6] = 100
    # ALl generators are on for off generators, please set the limits to 0.
    ppc['gen'][:, 7] = 1
    ...
    return ppc, matpower_data

required_input_names = ["Pd", "Qd", "Pg_DC", "Qg", "Vm", "Va"]
required_inputs = {name: globals()[name] for name in required_input_names}
ppc, matpower_data = build_required_inputs(**required_inputs)

data = ACOPFProblem(matpower_data, ppc)
...
quad_costs = np.array(data.quad_costs)
const_costs = np.array(data.const_costs)
...
nominal_pg = np.squeeze(np.array(nominal_pg[:, data.non_slack_gen_idx]))
nominal_vg = np.squeeze(np.array(nominal_vm[:, data.spv]))
nominal_pd = np.squeeze(np.array(data.X[sample_index, :data.nbus]))
nominal_qd = np.squeeze(np.array(data.X[sample_index, data.nbus:]))
...
Layer = PyomoOptLayer(model, variables_name, parameters_name, None, parameters_known)
...
class NNSolver(nn.Module):
    ...
    def forward(self, x):
        ...
        out[:, self._data.ng - 1:] = nn.Tanh()(out[:, self._data.ng - 1:])   # used to interpolate between max and min values
...
for i in range(nepochs):
    ...
    for Xtrain, Ytrain in train_loader:
        ...
        # used to satisfy the voltage magnitude constraints
        Yhat_train_partial_trans = data.complete_voltage(X_in, Yhat_train_partial).to(torch.float64)
        
        param_input = tuple([Yhat_train_partial_trans[:, :data.ng-1], Yhat_train_partial_trans[:, data.ng-1:], Xtrain[:, :data.nbus], Xtrain[:, data.nbus:data.nbus*2]])
        primal_batch, _, _, = Layer(*param_input)
        ...
        # model.vm, model.va, model.pg_ref, model.qg_all, model.qg_ref, model.vg, model.pg_all
    ...
    print('Epoch {}: train loss {:.4f}'.format(i, np.mean(batch_loss)), flush=True)
